...
"1F0E0011"# sampling logic and PARAMS
...
dc_reg = "FABCDABC"+"FF000010"+"FF050000"+"FF060000"+"FF0D0000"+\
...
for dcnum in range(1, 5):
	if asic_mask[4-dcnum] == '1':
		...
		if pedsub =="1":
			filename = "outdir/pedsub_data/DC%d_pedsub_data_%d_events.txt" %(dcnum,num_events)		
			if os.path.isfile("%s" %(filename)):
				print "Removing old pedsub file"
				os.system("sudo rm %s" %(filename))

			filename = "outdir/raw_data/DC%d_raw_data_%d_events.txt" %(dcnum,num_events)
			if os.path.isfile("%s" %(filename)):
				print "Removing old raw data file"
				os.system("sudo rm %s" %(filename))

		elif pedsub =="2":
			filename = "outdir/pedcalc_data/DC%d_ped_data_all_win.txt" %(dcnum)
			if os.path.isfile("sudo rm %s" %(filename)):
				os.system( "sudo rm %s" %(filename))
...
#					print trig_ch
					...
					if len(rcv)/2  > 28:
						...
						filename = "outdir/raw_data/event_trig_temp.dat"
						if os.path.isfile("%s" %filename):
							os.system("sudo rm %s" %(filename))
						f=open(filename,'ab')#set to 'ab' to append 
						f.write(rcv)
						f.close()
								
						# starting parser and ploting data
						os.system("python BMD_plot.py %s %s %d %d %d %d %d %d" %(filename,pedsub,plot_data,dcnum,srt_win,event,num_events,trig_ch))
						os.system("sudo rm %s" %(filename))
	...
	if os.path.isfile("sudo rm %s" %(filename)):
		os.system("sudo rm %s_asic_all.txt" %(filename))
...
#--------------------------------------------------------------------------------------------------------------------#
	...
	if calcMode == "2":#doing ped calc via software	
		...
		cmd_dc = "FABCDABC"+"FF000010"+"FF050000"+"FF060000"+"FF090000"+"FF0DFFF7"

	ctrl.send(syncwd+cmd_scrod+cmd_dc)
	time.sleep(.1)

	#sample offset used in parser
	sa_offset = 0
...
##			print cmd
			...
			outfile_name = 'outdir/raw_data/DC%d_raw_ped_data_%devents.dat' % (dcnum,num_events)
			if os.path.isfile("%s" % (outfile_name)):
				os.system("sudo rm %s" % (outfile_name))
			...
			for win_int in range(int(winStart/4),int(win_Offset/4)):
				...
				if calcMode == "1":#Doing ped calc via firmware and sneding back to PC
					...
					if len(rcv)/2 > 28:
						...
						outfile_name = 'outdir/pedcalc_data/DC%d_ped_data_wins_%d_to_%d.dat' % (dcnum,win_start,win_end)
						if os.path.isfile("%s" % (outfile_name)):
							print "Removing old ped data for windows %d to %d" %(win_start,win_end)
							os.system("sudo rm %s" % (outfile_name))
						...
						os.system("python BMD_plot.py %s %s %d %d %d 0 1 16" %(outfile_name,pedsub,plot_data,dcnum,win_start))
						os.system("sudo rm %s" %(outfile_name))
					else:
					...
					for event in range(num_events):
						...
						if len(rcv)/2  > 28:
							...
							if event%20 == 0:
								print "Current event %d for win %d to %d on DC#%d" %(event, win_start, win_end, dcnum)
						else:
							print "\nDC#%s: DEADBEEF" %(dcnum)

					os.system("python BMD_plot.py %s %s %d %d %d 0 %d 16" %(outfile_name,pedsub,plot_data,dcnum,win_start,num_events))
					os.system("sudo rm %s" %(outfile_name))	
...
#--------------------------------------------------------------------------------------------------------------------#
	...
	for dcnum in range(1, 5):
		if asic_mask[4-dcnum] == '1':
#			filename = "outdir/pedcalc_data/DC%d_ped_data_all_win_%devents.txt" %(dcnum,num_events)
			filename = 'outdir/pedsub_data/DC%d_pedsub_data_%d_events.txt' % (dcnum,num_events)
			os.system("python BMD_plot.py %s %s %d %d %d %d %d 16" %(filename,pedsub,plot_data,dcnum,winStart,0,num_events))
	...
	for event in range(num_events):
		for dcnum in range(1, 5):
			...
			if asic_mask[4-dcnum] == '1':
				...
				trig_ch = int(rcv[31:32],16)
#				print rcv
#				print "Received data: " + rcv[0:48]
#				print srt_win

				if len(rcv)/2  > 28:
					print "\nReadout of DC#%d windows %s to %s  event#%d complete" %(cur_dc,srt_win,srt_win+3,event)
					rcv = linkEth.hexToBin(rcv)
					#writing data to temp file
					filename = "outdir/raw_data/sw_trig_temp.dat"
					if os.path.isfile("%s" %filename):
						os.system("sudo rm %s" %(filename))
					...
					os.system("python BMD_plot.py %s %s %d %d %d %d %d 16" %(filename,pedsub,plot_data,cur_dc,srt_win,event,num_events))
					os.system("sudo rm %s" %(filename))
#					os.system("sudo rm %s_asic_all.txt" %(filename))
				else:
					print "\nDC#%s: DEADBEEF" %(dcnum)
