...
#remember to change the interface
...
port_pc = '28672'
port_fpga = '24576'
#all of these were set in the firmware so it's easier to keep these values here consistent

# Make UDP class for receiving/sending UDP Datagram
...
#-----------------------------------------------------------------------------------------------#
#------- Declaring a function to readback registers to make code shorter-----------------------#
#-----------------------------------------------------------------------------------------------#
def readReg(asic,RegNo):
	asic = int(asic)
    	dc_num = str(asic) #dc_num = hex(int(asic)).split('x')[1] #change made by tommy
...
def DCcheck(DCNUM):
	DCNUM = int(DCNUM)
	dc_num = str(DCNUM) #dc_num = hex(DCNUM).split('x')[1] #change made by tommy
	syncwd = "000000010253594e4300000000"
	buffSize = 200
	cmd1 = dc_num+"D070000"
	ctrl.send(syncwd+cmd1)
...
if com_type == "A" or com_type == "a" or com_type == "B" or com_type == "C":
    ...
    #major change by tommy here
    ##someone please check
    ###that would be much appreciated
    	if os.path.exists(BestCalib_file) == True: 
...
#				th[dcnum][ch]  = 1550	
        	...
        	print "The file %d does not exist. Will proceed forward" %BestCalib_file
...
##				temp = 0 #no need to zero it out since we overwrite it later anyways
			...
			for dcnum in range(1, 5):
				...
				if asic_mask[4-dcnum] == '1':
					print "\nStarting trigger rate scan for DC%s with best values from %sHz scan" %(dcnum, reg_num)
					...
					for ch in range(15):
						if th[dcnum-1][ch] == -1:
							...
							if THVal >= 4095:
								THVal = 4095
                            #NOTE: dcnum is an integer while cmd is a string
							cmd = str(dcnum)+"ABCDABC"+ str(dcnum)+hex(int("B000000",16)|(2*ch)*2**16|THVal).split('x')[1]+\
								   str(dcnum)+"F130001"+ str(dcnum)+"E000020"+ str(dcnum)+"F130000"
							ctrl.send(syncwd+cmd)
							time.sleep(0.5)
							...
							# setting threshold of current channel to max
							cmd = dcnum+"ABCDABC"+dcnum+hex(int("B000000",16)|(2*(ch))*2**16).split('x')[1]
							ctrl.send(syncwd+cmd)
							time.sleep(0.5)

					# scan complete programming threshold and dacs with calibrated values
					cmd = str(dcnum)+"ABCDABC" #modification by tommy
##  range(15)
					for ch in range(15):
						THVal = th[dcnum-1][ch]+ exth
						if THVal >= 4095:
							THVal = 4095
						cmd += dcnum+hex(int("B000000",16)|(2*ch)*(2**16)|THVal).split('x')[1]
#					print cmd
					ctrl.send(syncwd+cmd)
					#time.sleep(0.5)
...
#----------------------------------------------------------------------------------------------------#
		...
		# checking to make sure DAC values are within range
		...
		if reg_num == "hv" or reg_num == "HV":
		...
		#elif reg_num > "0" and reg_num < "16": #if we did it this way, we'd miss register "2"
			...
			if float(reg_val) <= 2.5:
				...
				dac_val = float(reg_val)*.0006105 ###why is this 6105 vs 6195
			...
			if float(reg_val) <= 2.5:
				dac_val = float(reg_val)
				for i in range(16):
					if i != 10: #i don't know if i understand why we ignore 10
...
##				cmd = DCNum+"ABCDABC"+DCNum+hex(int('D000000',16)|int(reg_num)*(2**16)).split('x')[1]
				cmd = str(dcnum)+"ABCDABC"+str(dcnum)+hex(int('D000000',16)|int(reg_num)*(2**16)).split('x')[1]
				ctrl.send(syncwd+cmd)
