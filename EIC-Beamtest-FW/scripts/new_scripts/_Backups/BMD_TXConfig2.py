...
"calc mode : 0 = Use fixed values [threshold 1550 abd DAC 0.9V]\n"+\
"            1 = Do initial or redo scan to find threshold base\n"+\
"                Note: Scan range HV-2V to HV-1V\n"+\
"            2 = Do scan to find best coincidence trigger rate\n"+\
"                Note: Input any offset to raw HV in to Exth\n"+\
"            3 = Program detector with best vlaues from file\n"+\
"                Note: Input any offset to best TH values in ExTh\n"+\
"            4 = Plot trigger rate vs HV\n"+\
"                Note: Input raw HV of file to plot\n"+\
"Base rawHV: 0-100\n"+\
"Ideal Rate: Ideal coincidance trigger rate\n"+\
"ExTH      : Extra threshold/HV offset for best values in file"
...
dac_LoVal   = 1 # dac scan lower limit set to ~0.8V(1300)
...
THCntOffset = 10  # TH offset from TH base found from scan, used for best trig rate count 
f0          = 25  # base trigger rate used in base scan [kHz]
plotmode    = 2   # 0:disable, 1:plot each ch on individual plot, 2:plot all ch on same plot
...
BaseCalib_file = "outdir/trim_dac_calb/BMD_BaseCalib_RawHV:%2.2fV.txt" %(rawHV)
caliball_file  = "outdir/trim_dac_calb/BMD_CalibAll_RawHV:%2.2fV.txt" %(rawHV)
BestCalib_file = "outdir/trim_dac_calb/BMD_BestCalib_Freq:%dHz.txt" %(IF0)
...
#--------------------------------------------------------------------------------------------#	
	...
	if not os.path.isfile(BaseCalib_file):
		...
		outfile.write("Note: For data set below HV bias range %2.2fV to %2.2fV(RawHV:%2.2fV) and trigger counts were counted %d above TH base," %(HV_LoLim,HV_UpLim,rawHV,THCntOffset))
		outfile.write("Scan was completed %s\n" %(time.strftime("%m/%d/%Y")))
		outfile.write("DC#\tCH#\tHV[V]\tT_cnt\tDAC[V]\tTH\n")
		outfile.close()
		...
		for ch in range(ChSrt,ChEnd):
			...
			for dac in range(dac_UpLim,dac_LoLim,-dac_delta):
				...
				print "**********Starting course threshold scan for all enabled DC**********"
				...
				for dcnum in range(1, 5):
					...
					if asic_mask[4-dcnum] == '1':
						if th_c[dcnum-1] == UpLim:
							...
							outfile = open(caliball_file, 'a')
							outfile.write("%d\t%d\t%.2f\t%d\t%.2f\t%d\n" %(dcnum,ch+1,float(rawHV-dacVal),0,dacVal,-1))
							outfile.close()
						else:
							quit_count = 0
							f_UpLim = th_c[dcnum-1]+3*th_delta 
							f_LoLim = th_c[dcnum-1]-2*th_delta
							if f_UpLim >= th_UpLim:
								f_UpLim = th_UpLim
							if f_LoLim <= th_LoLim:
								f_LoLim = th_LoLim

							print "**********Starting fine threshold scan for DC%d [TH range %d to %d]**********" %(dcnum,f_UpLim,f_LoLim)
							for th in range(f_UpLim,f_LoLim,-1):
								...
								if scalerCount > 0:
									if (freq > fmax[dcnum-1]):
										...
										quit_count += 1
										print "DC:%d CH:%d TH:%d freq:%4.2fkHz(%7d) fMax:%4.2fkHz qc:%d" %(dcnum,ch+1,th,freq,scalerCount,fmax[dcnum-1],quit_count)
								else:
									if quit_count > 0:
										quit_count += 1
							...
							outfile = open(caliball_file, 'a')
							outfile.write("%d\t%d\t%.2f\t%d\t%.2f\t%d\n" %(dcnum,ch+1,float(rawHV-dacVal),scalerCount,dacVal,THVal))
							outfile.close()
			...
			for dcnum in range(1, 5):
				...
				if asic_mask[4-dcnum] == '1':
					...
					if thBase[dcnum-1][ch] == -1 or hvBase[dcnum-1][ch] == -1 or thBase[dcnum-1][ch] == 1550 or hvBase[dcnum-1][ch] == int(0.95/.0006105):
						...
						f_UpLim = thBase[dcnum-1][ch]+2*ReTH_delta
...
#							print cmd
							...
							if scalerCount > 0:
								if abs(freq-f0) < minDeltaF:
									...
									print "New TH base value for DC%d CH%d, DAC:%1.2fV TH:%d freq:%4.2fkHz" %(dcnum,ch+1,hvBase[dcnum-1][ch]*0.0006015,th,freq)
...
#	cmd16 = "FABCDABC"+"FB1E0000"+hex(int('FC0A0000',16)|int((rawHV+HV0ffset)/0.02442)).split('x')[1]
	...
	for th in range(THRange):
		...
		ctrl.send(syncwd+cmd)
		time.sleep(1.5)			
		scalerCount = readReg(0,69)+readReg(0,96)*2**16
		freq = (scalerCount)
...
#------- plotting scan data -----------------------------------------------------------------#
#--------------------------------------------------------------------------------------------#
if PlotMode > 0 or CalbMode == 4: 
	for dcnum in range(1, 5):
		dc_num = hex(dcnum).split('x')[1]
		if asic_mask[4-dcnum] == '1':		
			CalData = [array('d'),array('d'),array('d'),array('d'),array('d'),array('d'),array('d'),array('d'),array('d'),array('d'),array('d'),array('d'),array('d'),array('d'),array('d')]
			DacVAl = array('d')
			...
			for line in csvFile:
				if CurLine > 1:
					if int(line[0]) == dcnum:
						CalData[int(line[1])-1].append(int(line[3])/1000)
			...
			if PlotMode == 1:
				...
				for i in range(ChSrt,ChEnd):#[0]: 
					plt.plot(DacVAl,CalData[i],color='black', linestyle='solid', marker='o',markerfacecolor='blue', markersize=5)
					plt.title('DC#%d CH#%d' %(dcnum,i+1))
					plt.xlabel("HV Bias[V]")
					plt.ylabel("Trigger count[10^3]")
					plt.ylim(-10,int(max(CalData[i]))+10)
					plt.xlim(max(DacVAl),min(DacVAl))

					plotName = 'outdir/trim_dac_calb/plots/DC%d_CH#%d_calib_RawHV:%2.2fV.png' % (dcnum,i+1,rawHV)	
				...
				f.text(0.02, 0.5, 'Trigger count[10^3]', va='center', rotation='vertical')
				MaxVal = 0
				for i in range(15):
					if MaxVal < int(max(CalData[i])):
						MaxVal = int(max(CalData[i]))

				for i in range(4):
					for j in range(4):
						if 4*i+j < 15:
							axarr[j,i].plot(DacVAl,CalData[4*i+j])
							axarr[j,i].axes.set_ylim(-20,MaxVal+10)
							axarr[j,i].axes.set_xlim(max(DacVAl),min(DacVAl))
							axarr[j,i].set_title('CH %d' %(4*i+j+1))

				plotName = 'outdir/trim_dac_calb/plots/DC%d_CalibAll_RawHV:%2.2fV.png' % (dcnum,rawHV)	
				plt.savefig(plotName)
				plt.clf()
