//compile independently with: g++ -o tx_ethparse1 tx_ethparse1.cpp `root-config --cflags --glibs`
#include <iostream>
#include <fstream>
#include <sstream>
#include <cstdlib>
...
//	if (count>2000){
	...
	if (DumpWaveform!=1 && DumpWaveform!=2)
	{
		for (int i=0;i<128;i++)
		{
			fprintf(outfile[fidx],"\n%u\t%u\t%u\t%u\t%u",curEvts,Wctime,i,asicNum,wraddrNum);
			for (int c=0;c<15;c++)	
				fprintf(outfile[fidx],"\t%d",+evtSa[i][c]-sa_offset*1);
			fprintf(outfile[fidx],"\t%d",+evtSa[i][15]-2048*1);
			fprintf(outfileCH,"%d,%d,%d\n",curEvts,i,+evtSa[i][14]-sa_offset*1); //just dump ch 12
		}
	}
