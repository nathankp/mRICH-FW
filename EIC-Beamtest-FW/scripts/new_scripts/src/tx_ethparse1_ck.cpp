...
//    sprintf(trigBitOutfile, "%s/triggerBits.txt", (char)working_dir);
    ...
    while (pos1 < (size-16)) {
        ...
        if ((buff[pos1]==0xFE && buff[pos1+4]==0xCF && (buff[pos1+8]&0xF0)==0xD0)) { // We do have a package.
            ...
            int wbuflen=wordNum+1; // Waveform buffer length as number of 32-bit words.
            ...
            evt->TrigCH     =   (buffer_uint[2])      & 0x0000000F ;   // word #2: read 5 bits
			evt->AddNum     =   (buffer_uint[2]>>8)   & 0x000001FF ;   // word #2: read 9 bits
			SrtWin          =   (buffer_uint[2]>>8)   & 0x000001FF ;  
			...
			if (OutMode == 1){//set OutMode to 1 to output format for chris scripts
		        outfile <<      evt->EvtNum      << " ";
		        outfile <<      evt->AddNum      << " ";
			...
			else if (OutMode == 2){//set OutMode to 2 output format for khanh scripts
				for (int j=0; j<128; j++){
				    outfile <<      evt->ASIC       << " ";					
				    outfile <<      SrtWin+j/32 	<< " ";
				    outfile <<      j		        << " ";
					for (int i=0; i<16; i++) outfile << evt->Sample[i][j] << " ";
					outfile << "\n";
				}
            }
			//adding last line into output file
			for (int i=0; i<20; i++){
				if (i == 0) outfile << evt->ASIC << "\t";
				else if (i == 1) outfile << SrtWin+3 << "\t";
				else if (i == 2) outfile << 128 << "\t";
				else if(i == 3) outfile << evt->TrigCH << "\t";
				else if(i == 4) outfile << evt->AddNum << "\t";
				else outfile << 0 << "\t";
			}
			outfile << "\n";
            eventCounter++;
